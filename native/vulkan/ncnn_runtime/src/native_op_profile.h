...
#else
    explicit ScopedNativeOpGpuTimestampQuery(NativeOpKind kind)
    {
        (void)kind;
    }

    template <typename CommandLike>
    ScopedNativeOpGpuTimestampQuery(NativeOpKind kind, CommandLike&)
        : ScopedNativeOpGpuTimestampQuery(kind)
    {
    }
#endif
...
#else
template <typename CommandLike, typename DeviceLike>
inline void finalize_native_op_gpu_profile(CommandLike&, const DeviceLike*)
{
}
#endif
