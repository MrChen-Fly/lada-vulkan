...
def blend_patch(
    ...
    if not restorer.runtime_features.use_vulkan_blend_patch:
        raise RuntimeError("Vulkan blend patch runtime is unavailable.")

    if restorer.runtime_features.use_vulkan_blend_patch_inplace:
...
def blend_patch_padded(
    ...
    if not restorer.runtime_features.use_vulkan_blend_patch_preprocess:
        raise RuntimeError("Vulkan padded blend patch runtime is unavailable.")

    pad_top, pad_bottom, pad_left, pad_right = [
        int(value) for value in pad_after_resize
    ]
    if restorer.runtime_features.use_vulkan_blend_patch_preprocess_inplace:
...
def blend_patch_padded_batch(
    ...
    if not restorer.runtime_features.use_vulkan_blend_patch_preprocess_batch_inplace:
        return None
    if not frame_rois:
        return []
