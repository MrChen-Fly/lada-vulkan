...
from lada.extensions.vulkan.basicvsrpp_blend import (
...
from lada.extensions.vulkan.basicvsrpp_cpu_extractor import runtime_value_to_numpy
...
from lada.extensions.vulkan.basicvsrpp_runtime_bootstrap import (
    ...
    def __init__(
        ...
        *,
        ...
        self.runtime_scheduling = RestorationSchedulingOptions(
            ...
            # NOTE:
            # The current Vulkan YOLO detector still shows history-dependent false positives
            # on some real clips when frames are fed in larger batches. Keep the detector
            # schedule conservative so Vulkan follows the verified per-frame semantics until
            # the NCNN/Vulkan state contamination is fixed at the detector runtime layer.
