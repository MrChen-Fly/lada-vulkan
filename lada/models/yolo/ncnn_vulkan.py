...
from lada.models.yolo.yolo11_segmentation_model import BaseYolo11SegmentationModel
...
def _normalize_imgsz(imgsz: int | tuple[int, int] | list[int]) -> tuple[int, int]:
    ...
    if isinstance(imgsz, int):
        return imgsz, imgsz
    if len(imgsz) != 2:
        raise UnsupportedComputeTargetError(f"Unsupported NCNN detection image size '{imgsz}'.")
...
def _precision_artifact_dir(model_path: Path, *, fp16: bool) -> Path:
...
def _resolve_existing_artifacts(
    ...
    *,
    ...
    candidate_dir = _precision_artifact_dir(model_path, fp16=fp16)
...
def _export_ncnn_artifacts(
    ...
    *,
    ...
    target_dir = _precision_artifact_dir(model_path, fp16=fp16)
...
class NcnnVulkanYoloSegmentationModel(BaseYolo11SegmentationModel):
    ...
    def _letterbox_to_ncnn_mat(self, image: Image | ImageTensor):
        ...
        target_h, target_w = _normalize_imgsz(self.imgsz)
    ...
    def _letterbox_to_vulkan_tensor(self, image: Image | ImageTensor):
    ...
    def _run_fused_subnet(self, input_frame: Any, orig_shape: tuple[int, int]) -> dict[str, np.ndarray]:
    ...
    def _run_fused_subnet_batch(
        ...
        if self.gpu_runner is None or self.postprocess_runner is None:
            raise RuntimeError("NCNN Vulkan detection requires the fused YOLO postprocess subnet.")
        prepared_batch = [self._to_ncnn_input_mat(input_frame) for input_frame in input_batch]
        if not hasattr(self.gpu_runner, "run_yolo_segmentation_subnet_batch"):
            return [
                self._run_fused_subnet(input_frame, orig_shape)
                for input_frame, orig_shape in zip(prepared_batch, orig_shapes)
            ]
        ...
        processed_batch = self.gpu_runner.run_yolo_segmentation_subnet_batch(
            ...
            self.postprocess_runner,
            [int(self.imgsz[0]), int(self.imgsz[1])],
            [[int(shape[0]), int(shape[1])] for shape in orig_shapes],
            float(self.args.conf),
            float(self.args.iou),
            int(self.args.max_det),
            len(self.names),
    ...
    def _warmup(self) -> None:
        ...
        dummy = torch.zeros((3, *self.imgsz), dtype=torch.float32)
        self._run_raw_outputs_to_cpu(dummy)
        self._run_fused_subnet(dummy, tuple(int(v) for v in self.imgsz))
        if hasattr(self.gpu_runner, "run_yolo_segmentation_subnet_batch"):
            self._run_fused_subnet_batch([dummy], [tuple(int(v) for v in self.imgsz)])
